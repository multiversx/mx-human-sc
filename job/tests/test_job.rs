...
use conftest::*;
use job::{
    base::JobBaseModule, constants::UrlHashPair, contract_obj, status::EscrowStatus, JobContract,
};
use multiversx_sc::{
...
use multiversx_sc_scenario::{
    managed_address, managed_biguint, managed_buffer, multiversx_chain_vm::tx_mock::TxContextRef,
    rust_biguint,
};
    ...
    blockchain_wrapper
        .execute_tx(
            ...
            |sc| {
                ...
                let new_callers: Vec<ManagedAddress<TxContextRef>> = vec![
                    managed_address!(&reputation_oracle),
                    managed_address!(&recording_oracle),
                ];
                let has_all_callers = new_callers
                    .iter()
                    .all(|caller| sc.trusted_callers().contains(caller));
    ...
    blockchain_wrapper
        .execute_query(contract_wrapper, |sc| {
            ...
            let expected_results =
                UrlHashPair::new(managed_buffer!(test_url), managed_buffer!(test_hash));
...
fn test_bulk_payout_partial() {
    ...
    blockchain_wrapper
        .execute_tx(
            ...
            &rust_biguint!(0u64),
            |sc| {
                let mut payments: MultiValueEncoded<
                    TxContextRef,
                    (ManagedAddress<TxContextRef>, BigUint<TxContextRef>),
                ...
                let final_results: UrlHashPair<TxContextRef> =
                    UrlHashPair::new(managed_buffer!(test_url), managed_buffer!(test_hash));
